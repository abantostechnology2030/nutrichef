...
- **⚠️ La IA de producción se configura en la BD, no en el `.env`.** Verificado el 2026-07-27: producción está en **`ai_modo='gemini'` con `ai_prioridad='gemini'`** (antes decía aquí `'ambos'` — la doc estaba desactualizada; lo que manda es `SELECT * FROM config WHERE clave LIKE 'ai_%'`, no este archivo). Genera en **~3,5 s y cuesta ~$0.038–0.045/semana**, contra ~30 s y ~$0.14 con Claude (sonnet-5 en precio de lanzamiento; sube a ~$0.21 el 1-sep-2026). Si alguien cambia la prioridad desde el admin, el costo se multiplica por ~4 (y por ~10 si lo pone en opus-4-8). El desglose por proveedor está en `GET /api/admin/resumen`.
  > ⚠️ **Con `ai_modo='gemini'` NO hay fallback.** Si Gemini falla (429, JSON malformado del gateway), la llamada devuelve 502 y el usuario ve el error; con `'ambos'` Claude la salvaba a cambio de ~4x el costo de esa llamada. Es una decisión de costo contra disponibilidad — sabe cuál tienes puesta antes de diagnosticar un 502 "raro".
